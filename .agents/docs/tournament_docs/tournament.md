...
| `/upcoming` | GET | Danh sách giải sắp diễn ra (chưa mở đăng ký) |
| `/{tournamentId}` | GET | Chi tiết giải đấu |
| `/{tournamentId}/participants` | GET | Danh sách người chơi đã đăng ký |
| `/{tournamentId}/participants/{participantId}` | GET | Chi tiết 1 người chơi trong giải |
...
| `/matches/{matchId}` | GET | Chi tiết 1 trận đấu |
...
Lấy danh sách giải sắp diễn ra — các giải đang ở trạng thái `Draft` hoặc chưa tới hạn đăng ký, dùng để player xem trước.

**Query:**

| Param | Type | Required | Mô tả |
|-------|------|----------|--------|
| `gameTemplateId` | Guid | ❌ | Mã game template (lọc theo game, optional) |
...
**Phân biệt với `/open`:**
- `/open` → `Status = RegistrationOpen`, deadline chưa qua, còn slot (player có thể đăng ký).
- `/upcoming` → `Status = Draft` hoặc chưa tới giờ mở đăng ký (chỉ xem trước).
...
## GET /api/v1/tournaments/{tournamentId}/participants/{participantId}

Chi tiết 1 người chơi trong giải — bao gồm Karma snapshot, Elo snapshot, trạng thái check-in, Swiss score, lịch sử trận của player đó trong giải.

**Path params:**
- `tournamentId` (Guid)
- `participantId` (Guid)

**Response 200:** `TournamentParticipantResponseDto`.

**Lỗi:** `401` thiếu token; `404` không tìm thấy participant; `500` lỗi hệ thống.

**Use case mobile:** Bấm vào 1 participant trong danh sách → xem profile trong giải đó.
...
## GET /api/v1/tournaments/matches/{matchId}

Chi tiết 1 trận đấu (1 bàn đấu cụ thể) — bao gồm: danh sách players, Swiss score, trạng thái (Pending / Ongoing / Completed / Cancelled), kết quả (winner, scores, timestamps).

**Path param:** `matchId` (Guid)

**Response 200:** `TournamentMatchResponseDto` (chi tiết).

**Lỗi:** `401` thiếu token; `404` không tìm thấy match; `500` lỗi hệ thống.

**Use case mobile:** Bấm vào 1 bàn đấu trong bracket → xem chi tiết (đang chờ kết quả, ai thắng, tỷ số, thời gian bắt đầu/kết thúc).
